...
class Cart:
    ...
    # метод  добавления товара как в корзину
...
class ProductCartUser:
    """
    Класс корзины для авторизованного пользователя
    """
    def __init__(self, request):
        # получаем текущего пользователя
        self.user = request.user
        ...
        # получаем позиции товаров вкорзине
    ...
    def save(self):
        for prod_id in self.cart:
            ...
            # иначе создаем новую позицию
                ...
                CartItem.objects.create(cart=self.user_cart, product=product,
    ...
    def remove(self, product_id, request):
        ...
        cart_item = CartItem.objects.get(catr=cart_user, product=product)
        cart_item.delete()
